...
class AdjectiveQuizScreen extends StatefulWidget {
  final String title;
  final List<AdjectiveData> adjectiveList;

  const AdjectiveQuizScreen({super.key, required this.title, required this.adjectiveList});

  @override
  State<AdjectiveQuizScreen> createState() => _AdjectiveQuizScreenState();
}

class _AdjectiveQuizScreenState extends State<AdjectiveQuizScreen> {
  final TextEditingController _answerController = TextEditingController();
  final FocusNode _focusNode = FocusNode();
  final FlutterTts flutterTts = FlutterTts(); 
  
  List<AdjectiveData> _activeQueue = [];
  List<AdjectiveData> _incorrectQueue = [];
  ...
  void initState() {
    super.initState();
    _initTts(); 
    _activeQueue = List.from(widget.adjectiveList); 
  }
  ...
  void _checkAnswer() {
    ...
    String normalizeText(String text) {
      String normalized = text.replaceAll(' ', '').replaceAll('　', '').toLowerCase();
    ...
    final possibleAnswers = currentAdj.vocabulary.split('/');
    
    for (String ans in possibleAnswers) {
      String cleanAns = ans.replaceAll(RegExp(r'\+\s*noun', caseSensitive: false), '');
      cleanAns = normalizeText(cleanAns);
      
      if (userAnswer == cleanAns) {
        isTextMatch = true;
        break;
      }
    }
    ...
    if (!_isAnswered) {
      setState(() {
        ...
        if (isTextMatch) {
          ...
          _isCorrect = false;
          _totalWrong++;
          if (!_incorrectQueue.contains(currentAdj)) {
            _incorrectQueue.add(currentAdj);
          }
      ...
      child: Column(
        ...
        children: [
          ...
          if (_showHint)
            Center(
              child: Text(
                'Hint: starts with $firstCharHint...',
              ...
              Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => _speak(currentAdj.vocabulary),
                  ),
                ],
              ),
              if (!_isCorrect) ...[
