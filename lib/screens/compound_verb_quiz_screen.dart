...
class CompoundQuizQuestion {
  final CompoundVerbData patternData;
  final CompoundVerbExample example;
  CompoundQuizQuestion(this.patternData, this.example);
}

class CompoundVerbQuizScreen extends StatefulWidget {
  final List<CompoundVerbData> verbDataList;

  const CompoundVerbQuizScreen({super.key, required this.verbDataList});

  @override
  State<CompoundVerbQuizScreen> createState() => _CompoundVerbQuizScreenState();
}

class _CompoundVerbQuizScreenState extends State<CompoundVerbQuizScreen> {
  final TextEditingController _answerController = TextEditingController();
  final FocusNode _focusNode = FocusNode();
  final FlutterTts flutterTts = FlutterTts(); 
  
  List<CompoundQuizQuestion> _activeQueue = [];
  List<CompoundQuizQuestion> _incorrectQueue = [];
  ...
  void _generateQuestions() {
    List<CompoundQuizQuestion> questions = [];
    for (var verb in widget.verbDataList) {
      for (var ex in verb.examples) {
        questions.add(CompoundQuizQuestion(verb, ex));
      }
    }
    questions.shuffle(); 
  ...
  void _checkAnswer() {
    ...
    String normalizeText(String text) {
      String normalized = text.replaceAll(' ', '').replaceAll('　', '').replaceAll('。', '').replaceAll('、', '').replaceAll('「', '').replaceAll('」', '').toLowerCase();
    ...
    final correctAnswer = normalizeText(currentQ.example.japanese);
    ...
    if (!_isAnswered) {
      ...
      if (_autoPlayAudio) {
        _speak(currentQ.example.japanese); 
      }
      ...
      child: Column(
        ...
        children: [
          ...
          if (_showHint)
            Center(
              child: Text(
                'Hint: starts with ${currentQ.example.japanese.substring(0, 2)}...',
              ...
              Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => _speak(currentQ.example.japanese),
                  ),
                ],
              ),
              if (!_isCorrect) ...[
                ...
                Text(
                  currentQ.example.japanese,
                  style: TextStyle(color: finalTextColor, fontSize: 22.0, fontWeight: FontWeight.bold),
                ),
              ],
              const SizedBox(height: 24.0),
              SizedBox(
